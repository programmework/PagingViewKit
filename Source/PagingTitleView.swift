...
@objc public protocol PagingTitleViewDelegate: NSObjectProtocol {
    /// 获取当前选中标题下标值
    @objc optional func pagingTitleView(pagingTitleView: PagingTitleView, index: Int)
}
...
public class PagingTitleView: UIView {
    ...
    private weak var delegate: PagingTitleViewDelegate?
...
// MARK: 外部方法：修改指示器颜色、标题颜色、标题文字相关方法
public extension PagingTitleView {
    /// 根据标题下标值重置标题文字
...
private extension PagingTitleView {
    @objc func btn_action(button: UIButton) {
        ...
        if (delegate != nil) && (delegate?.responds(to: #selector(delegate?.pagingTitleView(pagingTitleView:index:))))! {
            delegate?.pagingTitleView!(pagingTitleView: self, index: button.tag)
        }
